 ...
 * @version $Id: _E_Set.template,v 1.1.2.5 2009/09/15 02:38:31 upholderoftruth
 *          Exp $
 */
...
public interface TLongSet extends TLongCollection, Serializable {
	...
	/**
	 * Tests the set to determine if all of the elements in <tt>array</tt> are
	 * present.
	 * 
	 * @param array
	 ...
	 * <tt>true</tt> if the specified object is also a set, the two sets have the
	 ...
	 * that <tt>s1.equals(s2)</tt> implies that
	 ...
	 * following the end of the set is set to <tt>{@link #getNoEntryValue()}</tt>.
	 * (This is useful in determining the length of this set <i>only</i> if the
	 * caller knows that this set does not contain any elements representing
	 * null.)
	 * <p>
	 * If the native array is smaller than the set size, the array will be filled
	 * with elements in Iterator order until it is full and exclude the remainder.
	 * <p>
